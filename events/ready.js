module.exports = async (client, message) => {
    ...
    client.sql.query("Select * from Members where MutedUntil IS NOT NULL AND MutedUntil > ?", [Date.now()], (err, res) => {
        if (err) return console.error(err);

        let mutes = res.filter(r => r.MutedUntil);

        mutes.forEach(r => {
            const muteRole = client.guilds.cache.get(guild).roles.cache.find(role => role.name === 'Muted')
            if (r.MutedUntil < new Date().getTime()) {
                client.guilds.resolve(guild).members.fetch(r.DiscordID).then(m => {
