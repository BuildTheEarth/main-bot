    ...
    async run(this: Command, client: Client, message: CommandMessage, args: Args) {
        ...
        if (subcommand == "list" && !(subcommandGroup)) {
            ...
            if (!subcommandGroup) {
                ...
                if (!(await ReactionRole.exists(realEmoji, messageProps.messageId))) return message.sendErrorMessage("rrGone")

                const rr = await ReactionRole.findOne({emoji: realEmoji, messageId: messageProps.messageId}).catch(noop)
                ...
                if (subcommand == "list") {
                    ...
                    if (subcommandGroup == "blacklist") {
                        roles = client.reactionRoles.get(`${emoji}.${messageProps.messageId}`)?.blackListedRoles
                    } else {
                        roles = client.reactionRoles.get(`${emoji}.${messageProps.messageId}`)?.requiredRoles
                    }
                    if (!roles) embedStr[currIdx] = (subcommandGroup == "blacklist")? "**No Blacklisted Roles**": "**No Whitelistd Roles**"
                    else {
                        for (const e of roles) {
                            if (embedStr[currIdx].length > 4000) {
                                embedStr.push("")
                    ...
                    for (const embed of embedStr) {
                        if (first) {
                            embeds.push({
                                name: (subcommandGroup == "blacklist")? "Blacklisted Roles": "Whitelisted RP;es",
                                description: embed,
                                color: hexToNum(client.config.colors.success)
                            })
                            first = false
                ...
                if (subcommandGroup == "blacklist") {
                    ...
                    if (subcommand == "add") {
                        const res = await ReactionRole.addBlacklistedRole(client, rr.id, blacklistRole.id)
                        if (res) return message.sendSuccessMessage("rrAddB")
                        else return message.sendErrorMessage("rrGone")
                    } else if (subcommand == "remove") {
                        const res = await ReactionRole.removeBlacklistedRole(client, rr.id, blacklistRole.id)
                ...
                if (subcommandGroup == "require") {
                    ...
                    if (subcommand == "add") {
                        const res = await ReactionRole.addBlacklistedRole(client, rr.id, requiredRole.id)
                        if (res) return message.sendSuccessMessage("rrAddB")
                        else return message.sendErrorMessage("rrGone")
                    } else if (subcommand == "remove") {
                        const res = await ReactionRole.removeBlacklistedRole(client, rr.id, requiredRole.id)
                        if (res) return message.sendSuccessMessage("rrRemB")
                        else return message.sendErrorMessage("rrNoExist")
                    }
                }
