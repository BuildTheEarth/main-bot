...
export default new ComponentHandler({
    ...
    async run(client: Client, interaction: Discord.Interaction) {
        ...
        if (interaction.user.id != blockPage.userID) {
            ...
            .reply({ content: "Sorry, but you did not create this menu!", ephemeral: true })
            .catch(noop)
        }
        ...
        if (blockPage.pendingAction == "NEXT") {
            ...
            if (!retVal) {
                return await interaction
                    .reply({ content: "Hey there! Enough pages for you!", ephemeral: true })
                    .catch(noop)
            }
        ...
        if (interaction.message.editable) {
            await interaction.update({fetchReply:true, components: blockPage.getButtons()}).catch(noop)
            await interaction.message.edit({embeds: blockPage.getEmbeds(interaction.user.tag)}).catch(noop)
        } else {
            interaction.reply({ content: "Sorry, but you cannot use this menu anymore!", ephemeral: true })
        }
    },
})